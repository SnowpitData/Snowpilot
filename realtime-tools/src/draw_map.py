...
def draw_map(save_file,
    ...
    **kwargs : dict
    ...
    map_loc = kwargs['state_data']['location']
    state = kwargs['state']
    start_date = kwargs['start_date']
    end_date = kwargs['end_date']
    method = 'count' if  kwargs['show_avs'] == 'Daily count mean' else 'AAI' 
    ...
    if data == []:
        ...
        _map = folium.Map(location=map_loc, 
                          zoom_start=8, 
        ...
        try: 
            _map = map_avalanches(_map, 
                                  avalanches, 
                                  **kwargs)
        except (ValueError,  IndexError): # No avalanche data avaliabole
            log.info(f'No avalanche data or state: {state} has no polygons')
        ...
        if dot_legend != '' or line_legend != '':
            legend_img = (dot_legend + '_' + line_legend + '_label.png').replace('__', '_').strip('_')
...
def main(args):
    ...
    with open(STATES_DATA_PATH, 'r') as sdf:
        state_data = json.load(sdf)
    ...
    if not os.path.isfile(save_file_path):
        ...
        if xml_data != []:
            ...
            pit_data = parse_pits_data(xml=xml_data,
                                       **params_dict)
        params_dict['state_data'] = state_data[state]
...
if __name__ == '__main__':
    ...
    parser.add_argument('-p', '--params', default='{"start_date":"2021-11-07","end_date":"2021-11-20","state":"CO","text_param":"","dot_color_param":"ECT","line_color_param":"","observer_group":"All","show_avs":"Daily count mean"}')
